...
%perform regression on d_1
%define x data: 
x_train = d_1(:, 1)'; %Input: time, dataset with noise level 1
x_test = d_truth(:, 1)';
...
for i = 2:100
    y_train = d_1(:,i)'; %Output: observed data (mag)
    %Train GRNN:
...
xlabel('time');
ylabel('mag');
title(['Observed data d_1, GRNN model  MSE = ',num2str(avrgMSE)]);
box on;
